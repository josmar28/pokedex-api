<script>
    ...
    $('#add-type-modal').on('hidden.bs.modal', function (e) {
  $(this)
    ...
    function fetchdata() {
    	var loading = '<div class="text-center">'+
					  ...
					  '</div>';
    	$('.type-body').html(loading);
    	$('.loading').show();
    	var url = "{{url('/fetch-type-lists')}}";
		$.ajax({
            url: url,
            type: 'GET',
    ...
    $('body').on('submit', '#add-type-form', function(e) {
        e.preventDefault();
    	$('.loading').show();
    	var url = "{{url('api/submit-add-type')}}";
        $(this).ajaxSubmit({
            url: url,
            type: 'POST',
            success: function(data){
